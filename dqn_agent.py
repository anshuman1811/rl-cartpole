...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, pretrainedWeightsFile='checkpoint.pth', train = True, debug=False):
        ...
        if self.train:
          ...
          self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)        # Optimizer for training
          ...
          self.qnetwork_local = QNetwork(state_size, action_size, seed).to(device)    # Local Q network
          self.qnetwork_local.load_state_dict(torch.load(pretrainedWeightsFile))      # Load pre trained weights for Q network from file if testing
    ...
    def learn(self, experiences, gamma):
        ...
        Q_tgt_next = self.qnetwork_target(next_states).detach().max(1)[0].unsqueeze(1)
        Q_tgt = rewards + gamma*Q_tgt_next*(1-dones)
        
        Q_exp = self.qnetwork_local(states).gather(1, actions)
        
        loss = F.mse_loss(Q_exp, Q_tgt)
        if self.debug:
            print("loss ", loss)
